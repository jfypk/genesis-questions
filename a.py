...
def findLongestSpecialWord(wordlist):
    ...
    reachedEndOfNewDict = False
    word = ""
    
    #find the length of longest word in the wordlist
    longestWordLength = findLongestWordLength(wordlist)
    lengthOfLongestKnownWord = 5 #COINS

    #while specialWord has not been found
    while True:
        
        #start from the midpoint of the longest known word and the longest word in dict. 
        # 
        # If we find a special word, then check the words of the next highest length. If we don't find a special word, then that word is the longest special word. 
        #
        # If we don't find a word, then longestWordLength = lengthOfWord
        #
        if specialWordFound: 
            lengthOfWord += 1
        else: 
            lengthOfWord = math.ceil((longestWordLength + lengthOfLongestKnownWord)/2)

        print("analyzing words of length " + str(lengthOfWord)) #for viz
        #create a new dictionary with words of that length.
        newDict = findWordsOfLength(wordlist, lengthOfWord)
        ...
        for entry in newDict:
            ...
            if pathToOExists or pathToAExists or pathToIExists:
                ...
                if entry.upper() == newDict[-1]:
                    print('reached end of dict')
                    reachedEndOfNewDict = True
        
        if reachedEndOfNewDict and specialWordFound:
            break
        #if special word has not been found at length x, then special word will not be in x+1. Thus, we can narrow our search to words between x and lengthOfLongestKnownWord
        elif reachedEndOfNewDict and not specialWordFound:
